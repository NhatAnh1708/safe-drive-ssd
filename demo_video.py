...
def demo_video(path):
    ...
    category_index = create_category_index_from_labelmap("data/label_map.txt", use_display_name=True)
    ...
    while (cap.isOpened()):
        ...
        if not check:
            ...
            count_safe = 0
        if count_safe >= math.ceil(fps):
            now = datetime.now()
            dt_string = now.strftime("%d/%m/%Y %H:%M:%S")
            cv2.putText(image_np, f"{dt_string}:ATTENTION ", (10, int(0.75*w)),
